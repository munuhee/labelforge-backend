...
export async function run(_req: Request, res: Response) {
  try {
    ...
    const t3 = await Task.create({ ...acBase, title: 'Register a new business account on PayPal Business Portal', description: 'Log into PayPal Business using test credentials, navigate to Business Account Setup, simulate registration.', status: 'submitted', priority: 0.88, difficulty: 'hard', languageTags: ['en', 'sw'], externalUrl: 'https://www.paypal.com/business', estimatedDuration: 45, annotatorId: odhiambo._id, annotatorEmail: odhiambo.email, startedAt: ago(h(5)), submittedAt: ago(h(1)), actualDuration: 42, notes: 'Completed all 4 registration steps.', activityLog: [{ action: 'claimed', userId: odhiambo._id.toString(), userEmail: odhiambo.email, timestamp: ago(h(5)) }, { action: 'submitted', userId: odhiambo._id.toString(), userEmail: odhiambo.email, comment: 'Extra OTP screen documented', timestamp: ago(h(1)) }] })
    const t6 = await Task.create({ ...acBase, title: 'Book a domestic flight on Delta Airlines website', description: 'Go to Delta.com, search for a New York LGA → Los Angeles LAX flight next week, select cheapest fare.', status: 'data-ready', isLocked: true, priority: 0.85, difficulty: 'medium', languageTags: ['en'], externalUrl: 'https://www.delta.com', estimatedDuration: 25, annotatorId: odhiambo._id, annotatorEmail: odhiambo.email, reviewerId: amina._id, reviewerEmail: amina.email, qualityScore: 93, feedback: 'All required steps captured clearly.', startedAt: ago(d(4)), submittedAt: ago(d(3) + h(3)), completedAt: ago(d(2)), signedOffAt: ago(d(2)), actualDuration: 22 })

    await Task.insertMany([
      { ...acBase, title: 'Search and add item to wishlist on Amazon', status: 'unclaimed', priority: 0.95, difficulty: 'easy', languageTags: ['en'], externalUrl: 'https://www.amazon.com', estimatedDuration: 20, sla: new Date(now + d(3)) },
      { ...acBase, title: 'Book a bus seat on Greyhound — New York to Boston', status: 'in-progress', priority: 0.90, difficulty: 'medium', languageTags: ['en', 'sw'], externalUrl: 'https://www.greyhound.com', estimatedDuration: 30, annotatorId: wanjiru._id, annotatorEmail: wanjiru.email, startedAt: ago(h(2)), sla: new Date(now + d(2)), activityLog: [{ action: 'claimed', userId: wanjiru._id.toString(), userEmail: wanjiru.email, timestamp: ago(h(2)) }] },
      { ...acBase, title: 'Compare motor insurance quotes on Progressive', status: 'in-review', priority: 0.85, difficulty: 'medium', languageTags: ['en'], externalUrl: 'https://www.progressive.com', estimatedDuration: 35, annotatorId: wanjiru._id, annotatorEmail: wanjiru.email, reviewerId: amina._id, reviewerEmail: amina.email, startedAt: ago(h(8)), submittedAt: ago(h(4)), actualDuration: 32 },
      { ...acBase, title: 'Download tax return transcript from IRS portal', status: 'revision-requested', priority: 0.82, difficulty: 'hard', languageTags: ['en'], externalUrl: 'https://www.irs.gov', estimatedDuration: 40, annotatorId: kipchoge._id, annotatorEmail: kipchoge.email, reviewerId: mutua._id, reviewerEmail: mutua.email, feedback: 'Navigation screenshots for steps 2–4 are missing.', startedAt: ago(d(2)), submittedAt: ago(h(10)), actualDuration: 35 },
      { ...acBase, title: 'Find and apply a discount coupon on Target website', status: 'rejected', priority: 0.80, difficulty: 'medium', languageTags: ['en', 'sw'], externalUrl: 'https://www.target.com', estimatedDuration: 28, annotatorId: njeri._id, annotatorEmail: njeri.email, reviewerId: mutua._id, reviewerEmail: mutua.email, feedback: 'Annotator used the search bar instead of navigating via the Offers section.', startedAt: ago(d(3)), submittedAt: ago(d(1) + h(8)), actualDuration: 30 },
      { ...acBase, title: 'Log in and check account balance on Chase Bank', status: 'unclaimed', priority: 0.78, difficulty: 'easy', languageTags: ['en'], externalUrl: 'https://www.chase.com', estimatedDuration: 15, sla: new Date(now + d(8)) },
    ])
    ...
    const tl4 = await Task.create({ ...tlBase, title: 'Book a ride via the Uber web booking portal', status: 'data-ready', isLocked: true, priority: 0.82, difficulty: 'easy', languageTags: ['en'], externalUrl: 'https://www.uber.com', estimatedDuration: 18, annotatorId: tlAnnotator1._id, annotatorEmail: tlAnnotator1.email, reviewerId: tlReviewer._id, reviewerEmail: tlReviewer.email, qualityScore: 91, feedback: 'Pickup, drop-off, and fare estimate clearly captured.', startedAt: ago(d(5)), submittedAt: ago(d(4) + h(2)), completedAt: ago(d(3)), signedOffAt: ago(d(3)), actualDuration: 16 })

    await Task.insertMany([
      { ...tlBase, title: 'Search and save property listings on Zillow', status: 'unclaimed', priority: 0.90, difficulty: 'easy', languageTags: ['en'], externalUrl: 'https://www.zillow.com', estimatedDuration: 25 },
      { ...tlBase, title: 'Order food on Uber Eats and reach the checkout confirmation', status: 'in-progress', priority: 0.85, difficulty: 'easy', languageTags: ['en'], externalUrl: 'https://www.ubereats.com', estimatedDuration: 22, annotatorId: tlAnnotator1._id, annotatorEmail: tlAnnotator1.email, startedAt: ago(h(1.5)) },
      { ...tlBase, title: 'Track a courier parcel on FedEx tracking portal', status: 'submitted', priority: 0.80, difficulty: 'easy', languageTags: ['en'], externalUrl: 'https://www.fedex.com/tracking', estimatedDuration: 15, annotatorId: tlAnnotator2._id, annotatorEmail: tlAnnotator2.email, startedAt: ago(h(3)), submittedAt: ago(h(0.5)), actualDuration: 14 },
      { ...tlBase, title: 'Search and book a hotel room on Booking.com', status: 'revision-requested', priority: 0.75, difficulty: 'medium', languageTags: ['en'], externalUrl: 'https://www.booking.com', estimatedDuration: 30, annotatorId: tlAnnotator2._id, annotatorEmail: tlAnnotator2.email, reviewerId: tlReviewer._id, reviewerEmail: tlReviewer.email, feedback: 'Amenities comparison screenshots are missing for the second hotel result.', startedAt: ago(d(2)), submittedAt: ago(d(1) + h(3)), actualDuration: 28 },
    ])
